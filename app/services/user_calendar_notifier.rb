class UserCalendarNotifier
  ...
  def perform(reservation)
    find_calendar_by(Rails.application.secrets.default_calendar).tap do |cal|
      unless cal.nil?
  ...
  def event(reservation)
    {
      ...
      description: "Book '#{reservation.book.title}' (ISBN: #{reservation.book.isbn})<br><a href='#{Rails.application.secrets.app_host}/books/#{reservation.book.id}'>link to book page</a>"
    }
  end
